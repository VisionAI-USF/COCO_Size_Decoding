...
def perform_5_2_CV(params):
    catName = params['category']
    accuracies = []

    for curCat in catName:
        fname = params['tmp_dir'] + '{}_accuracy.txt'.format(curCat)
        cur_acc = crossValidation(params,curCat)
...
# Check that there is more or less equal number of classes in test, train and validation data
...
def prepare_data(out_path, animal, batch_size, seed, val_seed, fold_num, params):
    ...
    with open(labels_path, 'r') as file:
        ...
        labels = [(lambda t: (t[0], int(t[1])))(x.split(':')) for x in labels]
    ...
    for name, labels in zip(['train', 'valid', 'test'], [labels_train, labels_valid, labels_test]):
        ...
        for batch_num in range(num_batches):
            ...
            batch_labels = ku.to_categorical(np.array(batch_labels)[:, 1].astype(np.int), 2)
...
def crossValidation(params, category):
    ...
    for i, seed in zip(range(5), params['seeds']):
        ...
        for j in range(2):
            ...
            resulting_acc.append(test_accuracy)
